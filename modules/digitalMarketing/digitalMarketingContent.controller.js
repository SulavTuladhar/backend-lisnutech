const digitalMarketingContent = require ('./../../models/digitalMarketingContent.model');
const fs = require('fs');
...
const insert = (req,res,next)=>{
    ...
    const newContent = new digitalMarketingContent({});
...
const find =(req,res,next)=>{
    var condition= {}
    digitalMarketingContent.find(condition)
        .sort({
            _id: -1
        })
...
const findById = (req,res,next) => {
    digitalMarketingContent.findById(req.params.id, function(err,content){
        if(err){
            return next(err);
        }
        if(!content){
...
const update = (req,res,next) =>{
    digitalMarketingContent.findById(req.params.id,function(err,page){
        if(err){
            return next(err)
        }
        if(!page){
...
function remove(req,res,next){
    digitalMarketingContent.findById(req.params.id, function(err,product){
        if(err){
            return next(err)
        }
        if(!product){
